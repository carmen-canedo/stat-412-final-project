...
baseball_data <- data.frame(Count = all_boys_athletes_states$Baseball,
                            State = tolower(all_boys_athletes_states$State))
...
```{r}
boys_basket_data <- data.frame(Count = all_boys_athletes_states$Basketball,
                            State = tolower(all_boys_athletes_states$State))
...
# 
# ```{r}
# temp <- participation_count_1971_2019$Year
#   
# temp <- temp %>% 
#   str_replace_all(pattern = "[[:punct:]]\\d{2}$", replacement = "") %>% 
#   as.integer()
# 
# participation_recent <- participation_count_1971_2019 %>% 
#   select(-Year) %>% 
#   bind_cols(temp) %>% 
#   rename(Year = `...4`)
# 
# participation_recent <- participation_recent %>% 
#   filter(Year > 2004)
# 
...
# ```{r}
# participation_recent %>% 
#   ggplot(aes(x = Year, y = `Girls Participants`)) +
#   geom_line()
# ```
